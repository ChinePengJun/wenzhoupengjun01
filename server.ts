...
if (settingsCount.count === 0) {
  const initialSettings = [
    ...
    { key: "stats", value: [
      { value: "15+", label: "行业经验", labelEn: "Years Experience", sub: "Since 2009" },
      { value: "5000+", label: "厂房面积", labelEn: "Factory Size", sub: "Square Meters" },
      { value: "100W+", label: "日均产能", labelEn: "Daily Capacity", sub: "Pieces/Day" },
      { value: "2000+", label: "全球客户", labelEn: "Global Clients", sub: "Worldwide" }
    ]},
    { key: "categories", value: [
      { id: "stickers", title: "3D滴胶贴纸", titleEn: "3D Epoxy Stickers", desc: "高透明度、耐黄变、立体感强", descEn: "High transparency, non-yellowing, strong 3D effect", image: "https://images.unsplash.com/photo-1572375927902-d623603d1977?auto=format&fit=crop&q=80&w=800" },
      { id: "labels", title: "不干胶标签", titleEn: "Custom Labels", desc: "耐高温、耐腐蚀、防水防油", descEn: "Heat-resistant, corrosion-resistant, waterproof", image: "https://images.unsplash.com/photo-1626863905121-3b0c0ed7b94c?auto=format&fit=crop&q=80&w=800" },
      { id: "packaging", title: "定制包装盒", titleEn: "Custom Packaging", desc: "精美印刷、结构稳固", descEn: "Exquisite printing, stable structure", image: "https://images.unsplash.com/photo-1589939705384-5185137a7f0f?auto=format&fit=crop&q=80&w=800" }
    ]},
    { key: "aboutTitle", value: "关于云浠包装" },
    { key: "aboutTitleEn", value: "About Yunxi Packaging" },
    { key: "aboutContent", value: "温州云浠包装有限公司坐落于中国印刷城——龙港。我们是一家集研发、生产、销售于一体的专业包装企业。公司引进先进的生产设备，拥有一支经验丰富的技术团队，致力于为客户提供高品质的3D滴胶贴纸、不干胶标签及各类定制包装解决方案。" },
    { key: "aboutContentEn", value: "Wenzhou Yunxi Packaging Co., Ltd. is located in Longgang, the Printing City of China. We are a professional packaging enterprise integrating R&D, production, and sales. The company introduces advanced production equipment and has an experienced technical team." }
  ];
...
async function startServer() {
  ...
  app.post("/api/products", (req, res) => {
    const { id, title, category, price, img, video_url, moq, description, isFeatured, tiers, specs } = req.body;
    if (id) {
      db.prepare("UPDATE products SET title = ?, category = ?, price = ?, img = ?, video_url = ?, moq = ?, description = ?, isFeatured = ?, tiers = ?, specs = ? WHERE id = ?")
        .run(title, category, price, img, video_url || null, moq, description, isFeatured ? 1 : 0, JSON.stringify(tiers), JSON.stringify(specs), id);
      res.json({ success: true, id });
    } else {
      const result = db.prepare("INSERT INTO products (title, category, price, img, video_url, moq, description, isFeatured, tiers, specs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
        .run(title, category, price, img, video_url || null, moq, description, isFeatured ? 1 : 0, JSON.stringify(tiers), JSON.stringify(specs));
  ...
  app.post("/api/banners", (req, res) => {
    const { id, title, title_en, subtitle, subtitle_en, img, link, order_index } = req.body;
    if (id) {
      db.prepare("UPDATE banners SET title = ?, title_en = ?, subtitle = ?, subtitle_en = ?, img = ?, link = ?, order_index = ? WHERE id = ?")
        .run(title, title_en, subtitle, subtitle_en, img, link, order_index, id);
      res.json({ success: true, id });
    } else {
      const result = db.prepare("INSERT INTO banners (title, title_en, subtitle, subtitle_en, img, link, order_index) VALUES (?, ?, ?, ?, ?, ?, ?)")
        .run(title, title_en, subtitle, subtitle_en, img, link, order_index);
      res.json({ success: true, id: result.lastInsertRowid });
    }
  });
