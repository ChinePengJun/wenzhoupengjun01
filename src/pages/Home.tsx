...
import { ArrowRight, Play, Star, Shield, Zap, Award, CheckCircle2, ChevronLeft, ChevronRight as ChevronRightIcon } from "lucide-react";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProducts(productsData.filter((p: Product) => p.isFeatured === 1));
        setBanners(bannersData.sort((a: Banner, b: Banner) => a.order_index - b.order_index));
      } catch (err) {
    ...
    <main>
      ...
      <section className="relative h-screen flex items-center overflow-hidden bg-yunxi-dark">
          ...
          {banners.length > 0 ? (
            ...
            >
              <img 
                ...
                alt={language === 'zh' ? banners[currentBanner].title : banners[currentBanner].title_en}
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
          <AnimatePresence mode="wait">
            <motion.div 
              key={banners.length > 0 ? banners[currentBanner].id : "default"}
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <span className="text-yunxi-red font-black tracking-[0.4em] uppercase text-sm">
                  {banners.length > 0 ? (language === 'zh' ? banners[currentBanner].subtitle : banners[currentBanner].subtitle_en) : t('home.hero.badge')}
                </span>
              </div>
              <h1 className="text-6xl md:text-8xl font-black text-white leading-[0.9] tracking-tighter mb-10 text-balance">
                {banners.length > 0 ? (language === 'zh' ? banners[currentBanner].title : banners[currentBanner].title_en) : (
                  <>
              ...
              <div className="flex flex-wrap gap-6">
                <Link to={banners.length > 0 && banners[currentBanner].link ? banners[currentBanner].link : "/products"} className="px-10 py-5 bg-yunxi-red text-white font-black rounded-full hover:bg-white hover:text-yunxi-dark transition-all flex items-center gap-3 group">
                  {t('hero.cta')}
        ...
        {/* Banner Controls */}
        {banners.length > 1 && (
          <div className="absolute bottom-10 right-6 md:right-20 z-20 flex items-center gap-4">
            <button onClick={prevBanner} className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-yunxi-dark transition-all">
              <ChevronLeft size={24} />
            </button>
            <div className="flex gap-2">
              {banners.map((_, i) => (
                <button 
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${currentBanner === i ? "w-8 bg-yunxi-red" : "bg-white/40"}`}
                />
              ))}
            </div>
            <button onClick={nextBanner} className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center text-white hover:bg-white hover:text-yunxi-dark transition-all">
              ...
              {products.map((product, i) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-yunxi-dark/5">
                    ...
                    <Link to={`/products/${product.id}`} className="text-sm font-black text-yunxi-dark hover:underline underline-offset-8">{t('products.viewDetails')}</Link>
                  </div>
                </motion.div>
              ))}
            </div>
